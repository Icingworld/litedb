...
StorageEngine::StorageEngine(std::filesystem::path data_directory, filesystem::FileSystem & filesystem) noexcept
    : data_directory_(std::move(data_directory)), filesystem_(&filesystem) {}
...
std::expected<void, StorageError> StorageEngine::create_collection(schema::CollectionSchema schema)
{
    ...
    if (collections_.contains(id)) return std::unexpected(make_error(StorageErrorCode::CollectionAlreadyExists, "Collection already exists"));
    if (filesystem_ == nullptr)
        return std::unexpected(StorageError {StorageErrorCode::StoreError, "Storage engine is not configured", StorageStoreErrorCode::InvalidStoreState});
    auto exists = filesystem_->exists(store_path(id));
    if (!exists) return std::unexpected(StorageError {StorageErrorCode::StoreError, std::move(exists.error().message), StorageStoreErrorCode::FileSystemError});
    if (*exists) return std::unexpected(make_error(StorageErrorCode::CollectionStoreAlreadyExists, "Collection store already exists"));
    auto created = StorageStore::create(store_path(id), id, *filesystem_);
    if (!created) return std::unexpected(from_storage_store_error(std::move(created.error())));
...
std::expected<void, StorageError> StorageEngine::open_collection(schema::CollectionSchema schema)
{
    ...
    auto exists = filesystem_->exists(store_path(id));
    if (!exists) return std::unexpected(StorageError {StorageErrorCode::StoreError, std::move(exists.error().message), StorageStoreErrorCode::FileSystemError});
    if (!*exists) return std::unexpected(make_error(StorageErrorCode::CollectionStoreNotFound, "Collection store not found"));
    auto opened = StorageStore::open(store_path(id), id, *filesystem_);
    if (!opened) return std::unexpected(from_storage_store_error(std::move(opened.error())));
...
{
    if (!collections_.contains(id)) return std::unexpected(make_error(StorageErrorCode::CollectionNotFound, "Collection not found"));
    collections_.erase(id);
    if (filesystem_ != nullptr) {
        auto removed = filesystem_->remove(store_path(id));
        if (!removed) return std::unexpected(StorageError {StorageErrorCode::StoreError, std::move(removed.error().message), StorageStoreErrorCode::FileSystemError});
    }
    return {};
}

bool StorageEngine::contains_collection(common::CollectionId id) const noexcept { return collections_.contains(id); }
    ...
    const schema::CollectionSchema & schema, const schema::RecordData & data) const
{
    ...
    for (std::size_t index = 0; index < data.values.size(); ++index) {
        ...
        if (value.is_null()) {
            if (!column.nullable()) return std::unexpected(make_error(StorageErrorCode::NullConstraintViolation, "Column cannot be null: " + column.column_name()));
            continue;
        }
        ...
        if (column.type().id == common::LogicalTypeId::Varchar && column.type().parameter &&
            std::get<std::string>(value.data()).size() > *column.type().parameter)
            return std::unexpected(make_error(StorageErrorCode::ValueTooLarge, "VARCHAR value exceeds declared length: " + column.column_name()));
        if (column.type().id == common::LogicalTypeId::Vector && column.type().parameter &&
            ...
            return std::unexpected(make_error(StorageErrorCode::TypeMismatch, "VECTOR dimension mismatch: " + column.column_name()));
    }
...
std::expected<schema::Record, StorageError> StorageEngine::get(common::CollectionId collection_id, common::RecordId record_id) const
{
    const auto it = collections_.find(collection_id);
    if (it == collections_.end()) return std::unexpected(make_error(StorageErrorCode::CollectionNotFound, "Collection not found"));
    auto result = it->second.store->get(record_id);
    if (!result) return std::unexpected(from_storage_store_error(std::move(result.error())));
    return std::move(*result);
}

std::expected<common::RecordId, StorageError> StorageEngine::insert(common::CollectionId collection_id, schema::RecordData data)
{
    ...
    common::CollectionId collection_id, common::RecordId record_id, schema::RecordData data)
{
    auto it = collections_.find(collection_id);
    if (it == collections_.end()) return std::unexpected(make_error(StorageErrorCode::CollectionNotFound, "Collection not found"));
    if (auto valid = validate(it->second.schema, data); !valid) return valid;
    auto result = it->second.store->update(record_id, std::move(data));
    if (!result) return std::unexpected(from_storage_store_error(std::move(result.error())));
    return {};
}

std::expected<void, StorageError> StorageEngine::erase(common::CollectionId collection_id, common::RecordId record_id)
{
    auto it = collections_.find(collection_id);
    if (it == collections_.end()) return std::unexpected(make_error(StorageErrorCode::CollectionNotFound, "Collection not found"));
...
void StorageEngine::clear() noexcept { collections_.clear(); }
