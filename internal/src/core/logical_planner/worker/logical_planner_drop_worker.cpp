...
std::unique_ptr<plan::LogicalPlan> LogicalPlannerDropWorker::plan_drop_database(
    const BoundDropDatabaseStatement & statement
)
{
    return std::make_unique<plan::DropDatabasePlan>(statement.database_id());
}

std::unique_ptr<plan::LogicalPlan> LogicalPlannerDropWorker::plan_drop_collection(
    const BoundDropCollectionStatement & statement
)
{
    return std::make_unique<plan::DropCollectionPlan>(statement.collection_id());
}

std::unique_ptr<plan::LogicalPlan> LogicalPlannerDropWorker::plan_drop_index(
    const BoundDropIndexStatement & statement
)
{
    return std::make_unique<plan::DropIndexPlan>(statement.index_id());
}

std::unique_ptr<plan::LogicalPlan> LogicalPlannerDropWorker::plan_drop_vector_index(
    const BoundDropVectorIndexStatement & statement
)
{
    return std::make_unique<plan::DropVectorIndexPlan>(statement.vector_index_id());
}
