...
std::expected<std::unique_ptr<ast::ExpressionNode>, ParserError> ParserExpressionWorker::parse_wildcard_or_column_reference()
{
    ...
    auto first = context_.consume(TokenType::Identifier, "Expected select item");
    if (!first.has_value()) [[unlikely]] {
        return std::unexpected(first.error());
    }
    ...
    if (context_.match(TokenType::Dot)) {
        ...
        column = std::string(second->value());
    } else if (context_.match(TokenType::LeftParen)) {
        ast::FunctionCallExpression::ArgumentList arguments;
        if (!context_.check(TokenType::RightParen)) {
            while (true) {
                auto argument = parse_expression();
