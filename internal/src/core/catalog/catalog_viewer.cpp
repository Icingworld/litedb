...
std::optional<const entry::DatabaseEntry &> CatalogViewer::find_database(std::string_view name) const
{
    return state_.find_database(name);
}

std::optional<const entry::DatabaseEntry &> CatalogViewer::find_database(common::DatabaseId id) const
{
...
std::optional<const entry::CollectionEntry &> CatalogViewer::find_collection(common::CollectionId id) const
{
    return state_.find_collection(id);
}

std::optional<const entry::ColumnEntry &> CatalogViewer::find_column(
...
std::optional<const entry::IndexEntry &> CatalogViewer::find_index(
    common::CollectionId collection_id,
    std::string_view name
) const
{
...
std::optional<const entry::VectorIndexEntry &> CatalogViewer::find_vector_index(common::VIndexId id) const
{
    return state_.find_vector_index(id);
}

std::vector<std::reference_wrapper<const entry::DatabaseEntry>> CatalogViewer::list_databases() const
{
...
std::vector<std::reference_wrapper<const entry::VectorIndexEntry>> CatalogViewer::list_vector_indexes(
    common::CollectionId collection_id
) const
{
