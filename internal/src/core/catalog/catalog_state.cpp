...
{
    if (empty(request.database_name)) [[unlikely]] {
...
std::expected<void, CatalogError> CatalogState::drop_database(const DropDatabaseRequest & request)
{
    ...
    if (database_key == database_keys_.end() || database_key->second != request.database_id) [[unlikely]] {
        return std::unexpected(
            make_error(CatalogErrorCode::InvalidState, "Database name index is inconsistent")
        );
    }
    if (std::ranges::count(database_ids_, request.database_id) != 1) [[unlikely]] {
        return std::unexpected(
    ...
    for (const auto collection_id : collection_ids) {
        const auto collection = find_collection(collection_id);
        if (!collection || collection->database_id() != request.database_id) [[unlikely]] {
            return std::unexpected(make_error(
...
)
{
    ...
    if (empty(request.collection_name)) [[unlikely]] {
        return std::unexpected(
    ...
    if (const auto existing = database->find_collection_id(key)) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::DuplicateCollection,
            "Collection already exists: " + request.collection_name
        ));
    }
    if (request.columns.empty()) [[unlikely]] {
        return std::unexpected(make_error(
    ...
    for (const auto & column : request.columns) {
        if (empty(column.name)) [[unlikely]] {
            return std::unexpected(
                make_error(CatalogErrorCode::InvalidArgument, "Column name cannot be empty")
            );
        }
        if (!column_keys.insert(common::normalize_identifier(column.name)).second) [[unlikely]] {
            return std::unexpected(
                make_error(CatalogErrorCode::DuplicateColumn, "Duplicate column: " + column.name)
            );
        }
        if (!valid_logical_type(column.type) ||
            (column.default_expression && !valid_default_expression(*column.default_expression))) [[unlikely]] {
            return std::unexpected(make_error(
                CatalogErrorCode::InvalidArgument,
                "Invalid column type or default expression: " + column.name
            ));
        }
        if (column.unique && column.type.id == common::LogicalTypeId::Vector) [[unlikely]] {
            return std::unexpected(make_error(
                CatalogErrorCode::InvalidArgument,
    ...
    if (!can_allocate(next_collection_id_) ||
        ...
        !can_allocate(next_index_id_, unique_column_count)) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::InvalidState,
...
) const
{
    ...
    for (const auto column_id : collection->column_ids()) {
        ...
        if (!column || column->collection_id() != id) [[unlikely]] {
            return std::unexpected(make_error(
                CatalogErrorCode::InvalidState,
        ...
        if (!indexed_column_id || *indexed_column_id != column_id) [[unlikely]] {
            return std::unexpected(make_error(
                CatalogErrorCode::InvalidState,
    ...
    for (const auto index_id : collection->index_ids()) {
        ...
        if (!index || index->collection_id() != id) [[unlikely]] {
            return std::unexpected(make_error(
                CatalogErrorCode::InvalidState,
        ...
        if (!indexed_index_id || *indexed_index_id != index_id) [[unlikely]] {
            return std::unexpected(make_error(
                CatalogErrorCode::InvalidState,
    ...
    for (const auto vector_index_id : collection->vector_index_ids()) {
        ...
        if (!indexed_index_id || *indexed_index_id != vector_index_id) [[unlikely]] {
            return std::unexpected(make_error(
                CatalogErrorCode::InvalidState,
...
)
{
    ...
    if (!indexed_collection_id || *indexed_collection_id != request.collection_id) [[unlikely]] {
        return std::unexpected(
...
)
{
    ...
    if (empty(request.index_name)) [[unlikely]] {
        return std::unexpected(
    ...
    if (collection->contains_index(key)) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::DuplicateIndex,
            "Index already exists: " + request.index_name
        ));
    }
    if (collection->contains_vector_index(key)) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::DuplicateIndex,
            "Index name already exists: " + request.index_name
        ));
    }
    if (request.kind != entry::IndexKind::BTree) [[unlikely]] {
        return std::unexpected(
    ...
    if (!column || column->collection_id() != request.collection_id) [[unlikely]] {
        return std::unexpected(
            make_error(CatalogErrorCode::ColumnNotFound, "Index column not found")
        );
    }
    if (column->type().id == common::LogicalTypeId::Vector) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::InvalidArgument,
            "Scalar index cannot contain a VECTOR column"
        ));
    }
    if (!can_allocate(next_index_id_)) [[unlikely]] {
        return std::unexpected(
...
std::expected<void, CatalogError> CatalogState::drop_index(const DropIndexRequest & request)
{
    ...
    if (!indexed_index_id || *indexed_index_id != request.index_id) [[unlikely]] {
        return std::unexpected(
    ...
    const auto column = find_column(column_id);
    if (!column || column->collection_id() != index->collection_id()) [[unlikely]] {
        return std::unexpected(
            make_error(CatalogErrorCode::InvalidState, "Index column reference is inconsistent")
        );
    }
    if (column->unique() && index->unique() &&
        index->name() == implicit_unique_index_name(column_id)) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::InvalidArgument,
...
)
{
    ...
    if (empty(request.vector_index_name)) [[unlikely]] {
        return std::unexpected(
    ...
    if (column->type().id != common::LogicalTypeId::Vector || !column->type().parameter ||
        *column->type().parameter == 0) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::InvalidArgument,
    ...
    if (request.kind != entry::VectorIndexKind::Hnsw ||
        static_cast<std::uint8_t>(request.metric) >
            static_cast<std::uint8_t>(entry::VectorDistanceMetric::Cosine)) [[unlikely]] {
        return std::unexpected(
    ...
    if (request.hnsw_options.max_neighbors == 0 ||
        ...
        request.hnsw_options.ef_search_default == 0) [[unlikely]] {
        return std::unexpected(
            make_error(CatalogErrorCode::InvalidArgument, "Invalid HNSW options")
        );
    }
    if (!can_allocate(next_vector_index_id_)) [[unlikely]] {
        return std::unexpected(
...
)
{
    ...
    if (!indexed_index_id || *indexed_index_id != request.vector_index_id) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::InvalidState,
...
std::expected<void, CatalogError> CatalogState::restore(const CatalogSnapshot & source)
{
    ...
    for (const auto & database_snapshot : source.databases) {
        if (database_snapshot.id == 0 || empty(database_snapshot.name)) [[unlikely]] {
            return std::unexpected(make_error(
        ...
        for (const auto & collection_snapshot : database_snapshot.collections) {
            if (collection_snapshot.id == 0 ||
                ...
                empty(collection_snapshot.name)) [[unlikely]] {
                return std::unexpected(make_error(
                    CatalogErrorCode::InvalidSnapshot,
                    "Invalid collection in catalog snapshot"
                ));
            }
            if (collection_snapshot.columns.empty()) [[unlikely]] {
                return std::unexpected(make_error(
                    CatalogErrorCode::InvalidSnapshot,
            ...
            if (rebuilt.collections_.contains(collection_snapshot.id) ||
                database_ptr->contains_collection(collection->key())) [[unlikely]] {
                return std::unexpected(make_error(
                    CatalogErrorCode::InvalidSnapshot,
            ...
            for (std::size_t ordinal = 0; ordinal < collection_snapshot.columns.size(); ++ordinal) {
                ...
                if (value.id == 0 || empty(value.name) || rebuilt.columns_.contains(value.id) ||
                    collection_ptr->contains_column(common::normalize_identifier(value.name))) [[unlikely]] {
                    return std::unexpected(make_error(
                        CatalogErrorCode::InvalidSnapshot,
                ...
                if (!valid_logical_type(value.type) ||
                    ...
                    (value.unique && value.type.id == common::LogicalTypeId::Vector)) [[unlikely]] {
                    return std::unexpected(make_error(
                        CatalogErrorCode::InvalidSnapshot,
            ...
            for (const auto & value : collection_snapshot.indexes) {
                if (value.id == 0 || value.column_id == 0 || empty(value.name) ||
                    ...
                    value.index_kind != entry::IndexKind::BTree) [[unlikely]] {
                    return std::unexpected(make_error(
                        CatalogErrorCode::InvalidSnapshot,
                ...
                if (!collection_columns.contains(value.column_id) || !column) [[unlikely]] {
                    return std::unexpected(make_error(
                        CatalogErrorCode::InvalidSnapshot,
            ...
            for (const auto & value : collection_snapshot.vector_indexes) {
                ...
                if (!column || column->type().id != common::LogicalTypeId::Vector ||
                    ...
                    value.ef_search_default == 0) [[unlikely]] {
                    return std::unexpected(make_error(
                        CatalogErrorCode::InvalidSnapshot,
    ...
    if (source.next_database_id <= max_database || source.next_collection_id <= max_collection ||
        ...
        source.next_vector_index_id <= max_vector_index) [[unlikely]] {
        return std::unexpected(make_error(
            CatalogErrorCode::InvalidSnapshot,
