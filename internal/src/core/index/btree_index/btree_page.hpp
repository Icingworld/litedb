...
// B+ 树内部排序键
// 使用 (key, record_id) 作为全序键，使重复标量键跨页时仍能精确路由。
...
public:
    ...
    // 按序插入叶子条目
    // 插入成功返回 true，完整内部键重复时返回 false
    bool insert(BTreeLeafEntry entry);

    // 删除精确匹配的叶子条目
    // 删除成功返回 true，条目不存在时返回 false
...
// B+ 树内部页条目
// separator 是 right_child_id 所指子树中的最小内部键。
...
// B+ 树内部页
// first_child_id 处理小于首个 separator 的键；每个条目的右子页处理大于等于对应 separator 的键。
