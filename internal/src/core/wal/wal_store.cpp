...
)
{
    ...
    auto opened = filesystem.open(path, filesystem::backend::FileOpenOptions {
        .access = filesystem::backend::FileAccess::ReadWrite,
        .create_mode = filesystem::backend::FileCreateMode::OpenOrCreate,
    });
    if (!opened) return std::unexpected(filesystem_error(std::move(opened.error())));

    auto size = opened->size();
    if (!size) return std::unexpected(filesystem_error(std::move(size.error())));
    if (*size == 0) {
        ...
        return std::unexpected(error(WalErrorCode::InvalidFormat, "WAL file header is truncated"));
    } else {
        WalCodec::FileHeader header {};
        auto read = opened->read_at(0, header);
        if (!read) return std::unexpected(filesystem_error(std::move(read.error())));
        if (*read != header.size()) {
            return std::unexpected(error(WalErrorCode::InvalidFormat, "WAL file header is truncated"));
        }
...
)
{
    ...
    if (!appended) return std::unexpected(filesystem_error(std::move(appended.error())));
    return *size;
}
...
std::expected<void, WalError> WalStore::flush_through(transaction::Lsn lsn)
{
    ...
    if (lsn < WalCodec::FileHeaderSize || lsn >= *size) {
        return std::unexpected(error(WalErrorCode::InvalidRecord, "Cannot flush through an unknown LSN"));
    }
...
std::expected<WalScanResult, WalError> WalStore::scan(bool truncate_incomplete_tail)
{
    ...
    while (offset < *size) {
        ...
        if (!header_read) return std::unexpected(filesystem_error(std::move(header_read.error())));
        ...
        if (record_size < WalCodec::RecordHeaderSize) {
            return std::unexpected(error(WalErrorCode::CorruptedRecord, "WAL record size is invalid"));
        }
        ...
        if (record_size > std::numeric_limits<std::size_t>::max()) {
            return std::unexpected(error(WalErrorCode::CorruptedRecord, "WAL record is too large"));
        }
    ...
    if (result.truncated_tail && truncate_incomplete_tail) {
        ...
        if (!truncated) return std::unexpected(std::move(truncated.error()));
    }
    return result;
}
...
std::expected<void, WalError> WalStore::truncate_tail(std::uint64_t valid_size)
{
    if (valid_size < WalCodec::FileHeaderSize) {
        return std::unexpected(error(WalErrorCode::InvalidRecord, "Cannot truncate WAL before its header"));
    }
    auto truncated = file_.truncate(valid_size);
    if (!truncated) return std::unexpected(filesystem_error(std::move(truncated.error())));
    auto synced = file_.sync_data();
    if (!synced) return std::unexpected(filesystem_error(std::move(synced.error())));
    return {};
}
