...
{
    return type.id == LogicalTypeId::Integer || type.id == LogicalTypeId::BigInt ||
...
implicit_cast_cost(const LogicalType & source, const LogicalType & target) noexcept
{
    ...
    if (source.id == LogicalTypeId::Vector && target.id == LogicalTypeId::Vector) {
        if (!source.parameter.has_value() || !target.parameter.has_value() ||
            source.parameter.value() == target.parameter.value()) {
            return 1;
        }
        return std::nullopt;
    }
    if (is_numeric(source) && is_numeric(target) && numeric_rank(source) <= numeric_rank(target)) {
...
bool can_compare(const LogicalType & left, const LogicalType & right, BinaryOperator op) noexcept
{
    ...
    const auto ordered = op == BinaryOperator::LessThan || op == BinaryOperator::LessThanOrEqual ||
                         op == BinaryOperator::GreaterThan ||
                         op == BinaryOperator::GreaterThanOrEqual;

    if (equality) {
        return same_type(left, right) || (is_numeric(left) && is_numeric(right)) ||
               (is_varchar(left) && is_varchar(right));
    }
    return ordered &&
           ((is_numeric(left) && is_numeric(right)) || (is_varchar(left) && is_varchar(right)));
}
