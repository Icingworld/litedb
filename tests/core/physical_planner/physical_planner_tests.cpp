...
void test_all_plan_dispatch_and_ownership()
{
    ...
    require(static_cast<const physical_planner::plan::InsertPlan &>(*insert).values().size() == 1,
    ...
    auto update = planner.plan(std::make_unique<UpdatePlan>(
        1,
    ...
    require(update_plan.root_operator().kind() == physical_planner::op::PhysicalOperatorKind::SeqScan,
            "UPDATE root lowering");

    auto delete_plan = planner.plan(std::make_unique<DeletePlan>(
        1,
        std::make_unique<LogicalScanOperator>(1)
    ));
    require(delete_plan->kind() == physical_planner::plan::PhysicalPlanKind::Delete, "DELETE dispatch");
    require(static_cast<const physical_planner::plan::DeletePlan &>(*delete_plan).root_operator().kind()
                == physical_planner::op::PhysicalOperatorKind::SeqScan,
            "DELETE root lowering");
    require(lower_kind(std::make_unique<QueryPlan>(std::make_unique<LogicalScanOperator>(1)))
