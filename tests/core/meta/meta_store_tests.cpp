...
void test_missing_file_and_roundtrip(const std::filesystem::path & path)
{
    ...
    require(replaced->has_value() && (**replaced).next_vector_index_id == 42,
            "meta snapshot replacement did not publish");
}
...
void test_checksum_and_trailing_data_rejected(const std::filesystem::path & path)
{
    ...
    require(!corrupted && corrupted.error().is(meta::MetaErrorCode::ChecksumMismatch),
    ...
    require(!trailing && trailing.error().is(meta::MetaErrorCode::InvalidFormat),
...
void test_load_error_codes(const std::filesystem::path & path)
{
    ...
    require(invalid_magic.error().is(meta::MetaErrorCode::InvalidFormat),
            "invalid magic error code mismatch");

    write_test_header(filesystem, path, meta_magic, 1);
    auto unsupported_version = store.load();
    require(!unsupported_version.has_value(), "unsupported version should fail");
    require(unsupported_version.error().is(meta::MetaErrorCode::UnsupportedVersion),
    ...
    require(truncated.error().is(meta::MetaErrorCode::UnexpectedEof),
    ...
    require(oversized_count.error().is(meta::MetaErrorCode::ValueTooLarge),
