...
void test_automatic_checkpoint_by_wal_size()
{
    const auto directory =
        std::filesystem::temp_directory_path() / "litedb_automatic_checkpoint_tests";
    std::filesystem::remove_all(directory);

    {
        auto engine = open_database(
...
void test_database_recovery_uses_configured_wal_limits()
{
    ...
    auto limited = database::DatabaseEngine::open(
        database::DatabaseConfig {
            .data_dir = directory,
            .wal_decode_limits = {
...
void test_wal_write_budget_prevents_unrecoverable_commit()
{
    ...
    require(
        collections.rows.empty(),
        "rejected over-budget transaction became visible after restart"
    );
}
...
int main()
{
    ...
    transaction::TransactionId checkpoint_transaction_id = transaction::InvalidTransactionId;
    {
        ...
        require(
            before.wal_generation == 1 && before.wal_size_bytes > wal::WalCodec::FileHeaderSize,
            "initial WAL observation mismatch"
        );
        const auto automatic_before = engine->observability();
        require(
            automatic_before.automatic_checkpoint_attempts == 0,
        ...
        require(
            after.completed_checkpoints == 1 && after.failed_checkpoints == 0,
            "checkpoint counters mismatch"
        );
        require(
            after.wal_size_bytes == wal::WalCodec::FileHeaderSize && after.reclaimed_wal_bytes > 0,
            "checkpoint WAL reclamation metrics mismatch"
        );
        require(
            after.checkpoint_transaction_id > transaction::InvalidTransactionId,
    ...
    require(
        scanned->maximum_transaction_id > checkpoint_transaction_id,
