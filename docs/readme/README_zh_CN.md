...
- DML 与 DDL 均使用 checksum WAL 和 crash-consistent 的隐式语句级提交协议；Meta、Storage、B+Tree 与 HNSW 共享同一 Commit Record。
- 当前固定为全局单写者、语句级 Serializable；支持同步手动 checkpoint 和 generation WAL 回收，尚无自动/后台 checkpoint、显式事务、MVCC、细粒度锁或 compaction。
...
- 增加自动/后台 checkpoint、WAL 归档、更多数据 checksum、compaction 和文件格式版本管理。
